...
import com.smartCarSeatProject.activity.TestActivity;
...
public class Isometric {
    ...
    private final ColorM lightColorM;
    ...
    public Isometric() {
        ...
        this.lightColorM = new ColorM(255, 255, 255);
        this.currentWidth = -1;
    ...
    public void add(Path path, ColorM colorM) {
        addPath(path, colorM, null);
    }

    public void add(Path path, ColorM colorM, Shape originalShape) {
        addPath(path, colorM, originalShape);
    }

    public void add(Path[] paths, ColorM colorM) {
        for (Path path : paths) {
            add(path, colorM);
        }
    }

    public void add(Path[] paths, ColorM colorM, Shape originalShape) {
        for (Path path : paths) {
            add(path, colorM, originalShape);
        }
    }

    public void add(Shape shape, ColorM colorM) {
        /* Fetch paths ordered by distance to prevent overlaps */
        Path[] paths = shape.orderedPaths();

        for (Path path : paths) {
            addPath(path, colorM, shape);
        }
    }
    ...
    public void clearByTag(int iTag) {

        Iterator<Item> iterator = items.iterator();
        while (iterator.hasNext()) {
            Item item = iterator.next();
            Path path = item.getPath();
            if (path.getiTag() == iTag) {
                this.itemsChanged = true;
                iterator.remove();
            }
        }
    ...
    public void changeColorByTag(int iTag,ColorM colorM) {
        Iterator<Item> iterator = items.iterator();
        while (iterator.hasNext()) {
            Item item = iterator.next();
            Path path = item.getPath();
            if (path.getiTag() == iTag) {
                ColorM colorM1 = transformColor(path,colorM);
                item.setColor(colorM1);
            }
            else {
                ColorM colorM1 = transformColor(path, TestActivity.colorM);
                item.setColor(colorM1);
            }
        }
        this.itemsChanged = true;
    ...
    private void addPath(Path path, ColorM colorM, Shape originalShape) {
        this.itemsChanged = true;
        this.items.add(new Item(path, transformColor(path, colorM), originalShape));
    }

    /*private ColorM transformColor(Path path, ColorM color) {
        ...
        return color.lighten(brightness * this.colorDifference, this.lightColorM);
    }*/

    private ColorM transformColor(Path path, ColorM colorM) {
        ...
        return colorM.lighten(brightness * this.colorDifference, this.lightColorM);
    }
    ...
    public static class Item {
        ...
        Item(Item item) {
            ...
            this.path = item.path;
            this.baseColorM = item.baseColorM;
            this.originalShape = item.originalShape;
        }

        Item(Path path, ColorM baseColorM, Shape originalShape) {
            ...
            this.path = path;
            this.baseColorM = baseColorM;
            this.originalShape = originalShape;
            this.paint.setColor(android.graphics.Color.argb((int) baseColorM.a, (int) baseColorM.r, (int) baseColorM.g, (int) baseColorM.b));
        }
        ...
        public void setColor(ColorM baseColorM) {
            this.paint.setColor(android.graphics.Color.argb((int) baseColorM.a, (int) baseColorM.r, (int) baseColorM.g, (int) baseColorM.b));
        }
